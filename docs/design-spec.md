...
規則 2 的時間以 §4.6 的啟動統計所記錄的最後一次成功啟動時間為準；時間相同時依 stable ID 遞增排序，確保重新載入後順序決定性一致。此處**不**使用 `usage_score`——分數只用於 §4.5 的搜尋結果次要排序。需要固定位置的 App 應由使用者釘選（規則 1），釘選區永不被自動排序覆蓋。

規則：

- 同一 App 不可重複出現。
- 釘選與常用皆為空時，格狀區顯示一行提示（英文 UI 文字），不以任何其他 App 填充；空白狀態的內容一律只來自釘選清單與使用紀錄。
- 常用項目對應的 App 已不在 Catalog 中時不顯示，且其使用紀錄在下一次 Catalog 對帳時清除；釘選項目的缺席處理見 §FR-011。
...
- 釘選項目以左上角琥珀色截角標示，搜尋結果清單列以左緣琥珀色直條標示；形狀承載狀態、不以顏色單獨傳達（§NFR-006），高對比模式使用系統色。
...
- 當前 active（鍵盤選取）或 hover 的項目，其完整來源路徑顯示於面板底部的 footer band 左側（§4.9）。沒有檔案系統路徑的 packaged App 顯示來源標籤而非 Shell parsing name。path bar 與 footer 按鍵指引在面板高度被 clamp 縮短時仍完整可見（§4.9）。
- hover 只改變 path bar 內容與該格的淡填色，**不改變選取**；指標離開格狀區後 path bar 回到顯示選取項。`Enter` 永遠啟動具備選取邊框的那一格。
- 一次只顯示可見容量的格數；超出時除了 `PgUp`／`PgDn` 與滾輪，也在結果區右側顯示原生垂直捲軸。可見範圍的**起點**永遠對齊整列，不得從半列開始；最後一頁若資料不足，最後一列允許部分填入。
...
- 清單一次只顯示可見容量的列；超出時除了 `PgUp`／`PgDn` 與滾輪，也在結果區右側顯示原生垂直捲軸。
...
2. 名稱前綴相同。
3. 任一單字前綴相同。
4. 首字母縮寫相同：查詢字串是「名稱各單字首字母串接」的前綴（`vsc` 對 `Visual Studio Code`）。
5. 連續子字串相同。
6. 字元依序匹配（subsequence）。
7. 名稱完全不匹配，但次要比對鍵（§4.4）匹配。

縮寫層置於單字前綴之下、連續子字串之上（NR-203）：單字前綴比縮寫精確，故在其上；而 `vsc` 對 `Visual Studio Code` 是使用者刻意打的縮寫，對 `AvsCleaner` 只是字中意外撞到，故縮寫在連續子字串之上。缺了這一層，縮寫查詢會落到最低的 subsequence 層，與「字母碰巧散落」共用同一層，且因為使用分數只在同層內比較而永遠無法被提前。
...
- 單擊清單列或格子立即啟動。
- 滑鼠命中僅限實際繪製的可見格／列；footer 與邊緣空白按左鍵是拖曳面板，不啟動任何項目。
- 面板出現於游標所在螢幕工作區中央後，使用者可按住面板空白處拖曳移動；拖曳僅改變本次顯示的位置，不持久化。搜尋輸入框內的按壓一律屬於文字選取，不觸發拖曳。
...
- 面板空白處（格子之間、footer、結果區下方）的右鍵選單提供與通知區選單相同的「重新整理 App／設定／關於」三項；搜尋輸入框內的右鍵維持系統剪貼簿選單（§4.9）。
- 點擊面板外，面板自動隱藏。
- 滾輪只在結果超過可見容量時捲動。
...
- 預設寬度 640 DIP、高度 510 DIP，讓空白查詢狀態完整容納 6 欄 × 4 列。面板高度上限為目前螢幕工作區高度減 32px（work-32px 邊距）；完整 510 DIP 面板放不下時（小螢幕＋高 DPI），footer 提示帶恆貼齊面板底緣保持完整可見，改以縮減清單／格狀的可見列數容納，列高、格高、44 DIP footer 高度與面板寬度皆不變。
...
- footer 右側顯示當下適用的按鍵指引，左側在格狀狀態顯示 active／hover 項目的完整路徑（清單狀態留空）；不顯示狀態或版本資訊。footer 以比主體略深的藍白色帶區分，不繪製水平分隔線。捲動由結果區右側的原生捲軸提供，因此 footer 不再顯示 `PgUp`／`PgDn` 提示。路徑超出可用寬度時以尾端省略號截斷，且不得覆蓋右側指引。
...
其中「重新整理 App／設定／關於」三項與面板空白處的右鍵選單共用同一組命令與同一條分派路徑（§4.8）。
...
- 低階鍵盤 hook 僅限使用者主動開啟的快速鍵擷取小對話框（NR-089），以該對話框的生命週期為範圍安裝與移除；背景待機與一般快捷鍵偵測仍完全依賴 `RegisterHotKey`，不受影響。
...
4. 檔名主體為安裝工具產生的解除安裝殘骸者，無論副檔名一律排除：精確等於 `uninst`、`uninstall`、`uninstaller`，或 `unins` 後接一個以上且全為數字（`unins000`）。此處**不用前綴比對**——`unins*` 會連同 `Uninstall Tool`、`UninstallView`、`Uninstalr` 這類真正的解除安裝管理工具一起靜默排除。代價是 `Uninstall_ProductName.exe` 這類非固定命名的殘骸會漏出；多一列噪音是可見且可忽略的，誤殺一個真 App 則是靜默的，使用者只會發現「我的程式找不到」而無從得知原因。

採白名單而非黑名單的理由：黑名單的語意是「除了列舉到的以外都算程式」，遇到未預期的文件型別時**靜默漏出**；白名單直接表達「預設只看得到程式」。代價是冷門可執行型別（`.ps1`、`.vbs`、`.wsf`）會被排除，已知並接受。

套用範圍分成兩半（NR-202）：

- 判準第 1～3 條（AUMID、URL scheme、副檔名白名單）**不套用於 FR-005 的使用者自訂資料夾**。該來源的把關者是使用者自己勾選的副檔名清單；若對它二次過濾，使用者手動加入的副檔名會被無聲擋掉。
- 判準第 4 條（uninstaller 檔名）**一律套用**，三個來源都經過它。它只看檔名主體，與副檔名完全正交，擋不掉任何使用者手動加入的副檔名，因此上一條的理由不適用於它。缺了這一條，遞迴掃描 Program Files 這類目錄會把每個 Inno Setup／NSIS 安裝留下的 `unins000.exe`／`uninst.exe` 全部收進 catalog，在每一個能命中它們的查詢裡擠掉真正的 App。
...
- `.exe`、`.cmd`、`.bat` 只需是非目錄、非 reparse point 的檔案系統項目即可進入 catalog；是否真的可開啟交由 Shell 在啟動時判斷，與 `.lnk`、`.appref-ms` 一致。啟動失敗時走既有的啟動失敗處理（§FR-010）。
...
- 開機自動啟動。
- 跟隨系統／淺色／深色。
- 常用 App 顯示數量：1～1000，預設 20。設定頁輸入可解析的越界數值在欄位失去焦點（blur）時夾至端點；空值、非數字或解析溢位在 blur 時保留原文字，Save/OK 才走既有 validation。
- 啟動 App 後是否隱藏：預設開啟。
...
- 多個使用者自訂本機資料夾。自訂資料夾上限 32 個（與設定檔讀取端一致；超限的 `settings.ini` 視為損壞、退回預設並隔離原檔）；`hotkey` 值上限 256 字元，超限同視為損壞。
...
- AppsFolder 項目：能解析出檔案系統路徑時以該 resolved target 產生 stable ID，其餘情況以 AUMID／parsing name 產生。目的是讓同一個程式透過不同來源被找到時收斂為同一個身分。
- UserFolder `.exe`、`.cmd`、`.bat`：以正規化後的本機絕對路徑產生。
- UserFolder `.lnk`、`.appref-ms`：以捷徑自身的正規化路徑產生 stable ID，不解析其 target。該列舉器刻意不在掃描時使用 Shell COM，因此跨來源的去重在此較為保守。
...
「顯示一次錯誤提示」在本產品實作為**單一 tray balloon**，於主視窗與 tray icon 就緒後送出，一個 process 至多一則，多個檔案同時出問題時彙總為一則。
