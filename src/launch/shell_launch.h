...
// empty launch_identity (invalid/unresolved) or with launch_verified=false
// (NR-113: produced by catalog.cache, not yet revalidated by a current source
// enumeration) is rejected without any Shell call. Requires the calling thread
// to have initialized STA COM with COINIT_APARTMENTTHREADED |
// COINIT_DISABLE_OLE1DDE, as the UI thread does (design-spec §FR-010). No
// process handle is taken: the default path avoids SEE_MASK_NOCLOSEPROCESS, so
// the target process lifetime is the Shell's concern, not the caller's.
