...
- **NFR-001 formal probe** (`tests/release/nfr001_probe.ps1`): runs the long idle profile separately from the normal release runner. The default command is `pwsh -NoProfile -File tests/release/nfr001_probe.ps1`; it waits 60 seconds after startup readiness, samples idle CPU and memory for 15 minutes, probes cold `HotkeyReady` plus 100 warm `InputReady` samples, and completes a visible 20-row icon request cycle before measuring working set and `icons.cache`. Its output must be copied into `docs/performance-baseline.md` and `docs/release-evidence.md`; a window-ready timestamp, process-total thread count, or existing cache file alone does not satisfy the corresponding NFR gate.
