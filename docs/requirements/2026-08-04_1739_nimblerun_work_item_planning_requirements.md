...
- 使用者可設定多個本機資料夾，且每個資料夾可獨立選擇是否遞迴子目錄；Catalog 只掃描由使用者從受支援清單選取的可啟動副檔名，不能以此變成一般檔案搜尋。
...
- Catalog refresh 採啟動一次背景建置、檔案變更事件加 500 ms debounce、設定變更即時重建及 `Ctrl+R` 手動完整重建；面板顯示不做無條件完整掃描，AppsFolder 超過 10 分鐘才於叫出面板時背景更新。
- `ReadDirectoryChangesW` 只用變更種類過濾，不假設能以副檔名過濾；worker 依 allowlist 丟棄無關事件，overflow 時對來源做完整重掃。
