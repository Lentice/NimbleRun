...
public:
    ...
    // Entries of the first page of the empty-query view, capped at max_items.
    // Missing-pin placeholders are excluded because they have no icon. Empty
    // when the model currently has a non-empty query; never changes model state.
    std::vector<AppEntry> EmptyStatePrewarmEntries(std::size_t max_items) const;
