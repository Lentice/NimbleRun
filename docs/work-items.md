...
| NR-124 | §11 診斷缺口：損壞捷徑／缺失資料夾靜默略過、dedup 歧義計數丟棄 | 3 | `done` | NR-054, NR-063, NR-090, NR-091, NR-092 | [NR-124](work-items/NR-124-catalog-diagnostic-gaps.md) |
...
| NR-135 | `wWinMain` RAII 收尾（`HandleGuard`＋`ComGuard`）與刪除 `message_loop.h` | 3 | `done` | NR-051, NR-117 | [NR-135](work-items/NR-135-winmain-raii-and-message-loop-inline.md) |
...
| NR-183 | Explicit Refresh 繞過重掃節流：Ctrl+R 可被無限重複驅動全來源掃描 | 3 | `done` | NR-182 | [NR-183](work-items/NR-183-explicit-refresh-throttle.md) |
...
| NR-192 | Catalog rebuild worker 執行緒改為 background priority | 3 | `done` | NR-132 | [NR-192](work-items/NR-192-rebuild-worker-background-priority.md) |
...
| NR-201 | English input mode lifecycle seam：收斂暖機／顯示／focus 後切換契約 | 3 | `done` | NR-200 | [NR-201](work-items/NR-201-english-input-mode-lifecycle-seam.md) |
| NR-202 | Uninstaller 檔名判準：收窄前綴並套用到使用者自訂資料夾 | 5 | `done` | NR-019, NR-028, NR-193 | [NR-202](work-items/NR-202-uninstaller-stem-rule-scope.md) |
| NR-203 | 搜尋排名新增首字母縮寫層（initialism tier） | 5 | `done` | NR-047 | [NR-203](work-items/NR-203-initialism-match-tier.md) |
...
| 把 FR-004a 的**副檔名白名單**（判準第 1～3 條）套用到 FR-005 使用者自訂資料夾 | `docs/design-spec.md` §FR-004a | 該來源的把關者是使用者自己勾選的副檔名清單；二次過濾會無聲擋掉使用者手動加入的副檔名。**此欄仍然有效，不要重開。** ／ **2026-09-08 由 NR-202 部分覆寫**：原本這一列涵蓋整份判準，現已收窄為只涵蓋第 1～3 條。判準第 4 條（uninstaller 檔名）已套用到 FR-005，因為它只看檔名主體、與副檔名完全正交，本欄的否決理由不適用於它。新證據是使用者實機回報：遞迴掃描 `D:\Program files` 讓每個 Inno Setup／NSIS 留下的 `unins000.exe`／`uninst.exe` 殘骸進了 catalog，查詢 `uni` 的前 6 名全是殘骸。同時 NR-202 把該條由 `unins` 前綴收窄成精確清單，修掉它對 `Uninstall Tool`／`UninstallView`／`Uninstalr` 的既有誤殺。 |
...
- **1,000 次顯示／隱藏的 GDI／USER／Handle／Private Bytes 增量疑似非線性成長，需要乾淨環境重測**：
  design-spec §12.4 要求的洩漏測試已補上工具 `tests/release/leak_check.ps1`（重複
  ShowPanel／Esc 隱藏，`GetGuiResources`＋`Process.HandleCount`＋`PrivateMemorySize64`
  量前後差）。2026-08-26 在本機連續三次量測：100 次→handle +30／664 KiB，1,000 次→
  handle +9／228 KiB，2,000 次→handle +101／42 MiB——增量不隨次數單調成長，且 2,000 次
  那次的 Private Bytes 增量遠高於其他兩次。當時 `settings.ini` 的 `catalog_root` 指到
  `D:\Program files`（非使用者常見資料夾，量測期間該路徑之外的其他行程活動未受控），
  且量測機器同時有其他背景工作，懷疑是環境雜訊（背景 rebuild、其他行程競爭 handle 配額）
  蓋過真正的洩漏訊號，而非程式碼本身持續洩漏。要把這欄從「工具就緒、結果不可信」升級為
  「已量測、有結論」，需要在乾淨環境（無其他背景行程、`catalog_root` 指向次數少變動的
  資料夾）重跑至少兩組不同次數（如 500／2,000）並確認增量隨次數線性且收斂，或用該乾淨
  結果找出真正的洩漏來源再開 item。
...
- 2026-08-11（NR-164～NR-165 ready，第十六次全 repo 稽核產出）：以 codex backend 對整個 repo 做 fresh read-only audit（ponytail 過度設計／正確性／spec 符合度／安全四軸），主 Agent 對全部 finding 重讀原始碼驗證後收斂成 2 個 item、否決 1 個。**採納 NR-164（IMPORTANT）**——`IsLocalAbsolutePath`（`settings_store.cpp:73-75`）只委派給純字形檢查 `IsDisplayablePath`（`app_filter.cpp:26-35`），mapped SMB drive（`Z:\Apps`）通過驗證後被列舉／監看／stat，違反 FR-005（`design-spec.md:373`）「只接受本機磁碟路徑；拒絕 UNC、網路磁碟…」與 AGENTS「無網路」邊界。修法：只在設定驗證入口 `IsLocalAbsolutePath` 加 `GetDriveTypeW(DRIVE_REMOTE → false)`，抽純函式 `IsAcceptableDriveType` 供測試；**不做** `GetFullPathNameW` 正規化（root 字串改變會使 UserFolder stable id 失聯、破壞既有 pin／usage）；`IsDisplayablePath` 一字不改（每幀熱路徑，NR-144 收斂保留）。**採納 NR-165（LOW）**——`RecordLaunch`（`usage_store.cpp:166`）的 `++record.total_launches` 對 `UINT64_MAX` 無號回繞為 0：`ParseUint64` 接受完整 uint64 範圍（NR-070 只擋 `-` 前綴），手改／損壞的 usage.tsv 可觸發，計數歸零並經 `Save()` 自我永續、污染 `UsageScore` 與排序；採 saturating increment，不改 schema。**否決 codex finding 2（偽造 `kRefreshMessage` 觸發無限 rebuild）**——重開已否決方向無新證據：NR-130 明載「`kRefreshMessage`（Ctrl+R／tray）是使用者的顯式命令、無「來源」，不在限流範圍」；round-13 已接受 WM_APP 全家族同 user 偽造 DoS 結論（偽造 `kExitMessage` 可直接殺 process，比觸發 rebuild 更嚴重且已文件化接受）；`Start()` 內建 `Shutdown()` INFINITE join 是 NR-123「Ctrl+R 的 supersede join 維持原樣」的既有決策，偽造 WM_APP+2 的效果與 Ctrl+R 相同，無新增傷害。**刻意不開 item**：`IsDisplayablePath` 對 mapped drive 的顯示層用途（main.cpp 顯示決策不受影響）；usage.tsv 的 `UINT64_MAX` 走 corrupt 路徑（計數極大但格式合法，saturate 即可）。codex 環境無法執行 build／CTest（唯讀政策），報告以靜態追蹤為準；主 Agent 已逐行重讀 `settings_store.cpp`／`app_filter.cpp`／`usage_store.cpp`／`rebuild_pipeline.cpp` 相關段落驗證。未 commit。

- 2026-08-11（NR-166～NR-168 ready，第十六次稽核第 2 輪產出）：NR-164～165 完成後重跑
  同一 codex audit。主 Agent 重讀原始碼驗證後收斂成 3 個 item、全部完成。**採納
  NR-166（IMPORTANT）**——`ReadAllBytes`（`atomic_text_file.h:83-110`）只回 `bool`，
  size-cap 分支不設 last error、`ReadFile` 失敗後先 `CloseHandle` 才回傳，
  `ReadVersionedLines`（`:304-306`）事後讀 thread-local `GetLastError()` 分流
  Missing／Unreadable——執行緒 last-error 殘留 2／3 時超限/不可讀的既存檔被誤判
  `Missing`，繞過 `.corrupt` 保存與通知契約（NR-058/§10.4），後續 Save 直接覆寫
  原檔。修法：`ReadAllBytes` 增加 `DWORD* error_out`，三條失敗路徑各自保存錯誤碼
  （size cap 明確 `ERROR_FILE_TOO_LARGE`），呼叫端不再依賴事後讀 last error；
  `Missing` 只保留給 CreateFileW 真的回 2/3；不新增列舉值、store 回傳值不變；
  新增 stale-last-error 回歸測試（`SetLastError(ERROR_FILE_NOT_FOUND)` 後讀超限
  檔 → 非 Missing）。**採納 NR-167（LOW）**——`PostFlush`（`icon_worker.cpp:147-170`）
  的 `queue_.push_back` 無例外邊界（`Post()` 有、`PostFlush()` 漏），bad_alloc
  穿越 window proc 終止常駐 process；`HidePanel`（`main.cpp:938`）由 UI 訊息路徑
  直呼。修法：`PostFlush` 整個本體包 `try/catch (...)` 靜默 return（與 `Post()`
  同形、NR-099 註解逐字保留）。**採納 NR-168（LOW）**——`testing.md:34-35` 驗收
  清單要求「非空搜尋維持 grid」，與 §4.3/§4.7 單欄清單矛盾（碼對、文件錯）；
  改兩行措辭。三項皆無依賴、獨立完成。
- 2026-08-11（NR-169～NR-171 ready，第十六次稽核第 3 輪產出）：第 2 輪完成後重跑
  同一 codex audit。主 Agent 重讀原始碼驗證後收斂成 3 個 item、全部完成。**採納
  NR-169（IMPORTANT）**——`PinStore::Load`（`pin_store.cpp:95-98`）的 dedup 集合
  `std::unordered_set<std::wstring_view> seen` 先插入指向區域變數 `pin.stable_id`
  的 view、再 move 進 `pins_`；SSO 短字串 move 是複製內嵌 buffer，view 在區域
  變數析構後懸空（UB），短 ID 完全合法且既有測試恰好全用短字串故測試照過。
  修法：`seen` 改 `std::unordered_set<std::wstring>`（擁有鍵值），usage_store 的
  對應迴圈已是擁有鍵值不需動；新增重複短 ID＋長 ID 測試。**採納 NR-170
  （IMPORTANT）**——`RebuildPipeline::Start` 的 worker 建立 catch（`rebuild_pipeline.cpp:
  143-146`）呼叫 `ApplySourceFailure` 後漏掉 `CompleteIfReady`（前置 setup failure
  路徑 `:99-106` 有）；單來源 rebuild（AppsFolder on-demand、watcher Change）
  建立 thread 失敗時 `on_complete_` 永不執行——snapshot refresh、診斷、cache
  commit、launch-failure gate 全數跳過，NR-106 契約的抽出後缺口。修法：catch 補
  `CompleteIfReady(generation)`；新增可注入 thread factory seam（`std::function`
  建構子參數、預設零變更）＋拋例外的 factory 測試斷言 completion 恰好一次。
  **採納 NR-171（LOW）**——`WM_SIZE` 分支（`main.cpp:2785`）直接採用 lParam 的
  16-bit 尺寸呼叫 `Resize` 且忽略 HRESULT；同 integrity process 可
  `SendMessageW(hwnd, WM_SIZE, 0, MAKELPARAM(65535,65535))` 要求約 4.29B pixels
  的 surface（NR-149 同一 HWND untrusted-input 邊界的尺寸資料版本）。修法：忽略
  lParam，改以 `GetClientRect` 取得實際 client size，`Resize` HRESULT 失敗寫既有
  診斷；真 WM_SIZE 的 lParam 本就等於 client rect，正常路徑零影響。**搜尋截斷
  1023 字元（`main.cpp:2525` 固定 `wchar_t buffer[1024]`）**維持 round-13 的低
  嚴重度備查決策不開 item（codex 第 4 輪重提時無新證據——同 integrity WM_SETTEXT
  屬已接受的 same-user 面，EM_SETLIMITTEXT 是選修的 UX 一致化，非重要缺陷）。
- 2026-08-11（NR-172～NR-175 ready，第十六次稽核第 4 輪產出）：第 3 輪完成後重跑
  同一 codex audit。主 Agent 重讀原始碼驗證後收斂成 4 個 item、全部完成。**採納
  NR-172（IMPORTANT）**——`IsLocalAbsolutePath`（NR-164 後）接受恰為 volume root
  的路徑（`C:\`），`AddRoot` 接受後以預設 `recursive=true` 整顆磁碟遞迴列舉＋
  recursive watcher，違反 §19.5（`design-spec.md:1082`「不掃描整顆磁碟」）；
  `kMaxCatalogRoots` 只限數量不限涵蓋範圍。修法：共用驗證入口拒絕 Trim 後恰三
  字元（`X:\`／`X:/`）的路徑，Load 與 AddRoot 自動同時生效，子資料夾照舊。
  **採納 NR-173（IMPORTANT）**——worker lambda（`rebuild_pipeline.cpp:121-125`）
  的 `const RebuildEnumeration enumeration` 後 `result->entries = enumeration.entries`
  是深拷貝（AppEntry 含多個 wstring），每次 rebuild 同時持兩份完整結果，記憶體
  峰值翻倍；與 NR-172 合併放大。修法：移除 const、`std::move(enumeration.entries)`
  與 diagnostics。**採納 NR-174（LOW）**——`DiagnosticLog::Write`（`diagnostic_log.cpp:
  33-88`）宣稱 never throws 但 mutex lock 與多個配置操作無例外邊界；錯誤記錄路徑
  在 OOM 時可讓例外穿越 WndProc 終止 process。修法：標 `noexcept`＋最外層
  `try/catch (...)` 空 catch return。**採納 NR-175（LOW）**——`WatchLoop`
  （`catalog_watcher.cpp:55-56`）thread body 首行配置 64 KiB vector，entry 無最
  外層例外邊界；NR-097 只蓋 thread 建立失敗。修法：body 包最外層 `try/catch (...)`
  catch 內僅 return（watcher 結束，`Stop()` 的 join 正常完成）。
- 2026-08-11（第十六次稽核第 5 輪，codex）：第 4 輪完成後重跑同一 codex audit，
  對照完整稽核歷史與已否決方向——**零 finding（clean review）**，無新的
  CRITICAL／IMPORTANT／可行動 MINOR。稽核迴圈於第 5 輪收斂（上限 5 輪）。本輪
  同時確認 NR-164～175 十一個修補皆落地；codex 環境為唯讀，build／CTest 由各
  subagent 實作時執行（每次 31/31 全綠）。
